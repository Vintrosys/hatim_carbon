class VisualPlantFloor {
	...
	make() {
		this.wrapper.append(`
			<div class="plant-floor">
				<div class="plant-floor-filter row"></div>
				<div class="plant-floor-container col-sm-12"></div>
			</div>
		`);
	...
	prepare_data() {
		frappe.call({
			...
			callback: async (r) => {
				...
				this.workstations = await this.filter_workstations_with_job_cards();
	...
	add_filter() {
		...
		this.workstation_status.$wrapper.addClass("form-column col-sm-2");
	}

	render_plant_visualization() {
		let plant_floor = this.plant_floor.get_value();

		if (!plant_floor) return;
	...
	filter_workstations_with_job_cards() {
			...
			frappe.call({
				...
				callback: (r) => {
					...
					const ws_set = new Set(
						r.message.map(jc => jc.workstation).filter(Boolean)
					);

					const filtered = this.workstations.filter(ws =>
						ws_set.has(ws.name)
					);
	...
	render_workstations() {
		const container = this.wrapper.find(".plant-floor-container");
		container.empty();

		if (!this.workstations.length) {
			container.html(`
				<div class="text-muted text-center" style="padding:20px;">
					No machines with Job Cards
				</div>
			`);
			return;
		}
